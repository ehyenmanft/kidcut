...
export class Exporter {
  ...
  async exportVideo(timelineEngine, duration, fps, totalFrames, canvas, ctx, format, onProgress) {
    ...
    const mediaRecorder = new MediaRecorder(stream, {
      mimeType,
      videoBitsPerSecond: 12000000 // 12 Mbps for crisp 8-bit pixel fidelity
    });
    ...
    for (let frame = 0; frame < totalFrames; frame++) {
      ...
      // Small pause to allow UI update and prevent locking low-spec CPU
      await new Promise(r => setTimeout(r, Math.max(1, Math.min(16, frameIntervalMs / 4))));
    }
